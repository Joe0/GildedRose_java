...
public class InventoryTest {
	...
	public void conjured_items_should_degrade_twice_as_fast() throws Exception {
		Item[] i = new Item[] { new Item("Conjured Mana Cake", 3, 6), new Item("Conjured Mana Cake", -1, 6) };
		Inventory inv = new Inventory(i);
		inv.updateQuality();
		assertEquals(4, i[0].getQuality());
		assertEquals(2, i[0].getQuality());
	}

	@Test
	public void conjured_items_should_have_bounds_checking() throws Exception {

		Item[] i = new Item[] { new Item("Conjured Mana Cake", 3, 1),
				new Item("Conjured Mana Cake", 3, 55) };
		Inventory inv = new Inventory(i);
		inv.updateQuality();
		assertEquals(0, i[0].getQuality());
		assertEquals(50, i[1].getQuality());
	}
}
